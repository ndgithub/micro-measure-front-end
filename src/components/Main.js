...
class Main extends React.Component {
  ...
  getInitSize = () => {
    var ratioY =
      this.origDims.height / this.state.containerRef.current.offsetHeight;
  ...
  getNewPosition = (containerRef, oldSize, oldPos, newSize) => {
    let newPos = {};
    let old_containerCenterX_relToImgCorner_percent =
      (containerRef.current.offsetWidth / 2 - oldPos.x) / oldSize.width;
    let old_containerCenterY_relToImgCorner_percent =
      (containerRef.current.offsetHeight / 2 - oldPos.y) / oldSize.height;

    let new_pointToKeepCenteredX_ReltoContainer_pixels =
      old_containerCenterX_relToImgCorner_percent * newSize.width + oldPos.x;
    let offsetX =
      new_pointToKeepCenteredX_ReltoContainer_pixels -
      containerRef.current.offsetWidth / 2;
    newPos.x = oldPos.x - offsetX;

    let new_pointToKeepCenteredY_ReltoContainer_pixels =
      old_containerCenterY_relToImgCorner_percent * newSize.height + oldPos.y;
    let offsetY =
      new_pointToKeepCenteredY_ReltoContainer_pixels -
  ...
  onClickDoneSetting = () => {
    let imgScalePerc = Math.abs(
      this.state.scalePts[0].x - this.state.scalePts[1].x
    );
  ...
  wasDragged = () => {
    let dragged =
      this.lastMouseDownPos.x !== this.lastMouseUpPos.x &&
      this.lastMouseDownPos.y !== this.lastMouseUpPos.y;
  ...
  convertToImgPos = pagePos => {
    ...
    var imgPtX =
      (pagePos.x -
        this.state.containerRef.current.offsetLeft -
        this.state.pos.x) /
      this.state.size.width;
    var imgPtY =
      (pagePos.y -
        this.state.containerRef.current.offsetTop -
        this.state.pos.y) /
  ...
  onSaveSnapClicked = () => {
    html2canvas(this.state.containerRef.current, {
      logging: false,
      useCORS: true
    }).then(canvas => {
  ...
  mouseDown = e => {
    ...
    if (this.state.isDrawLineinProg) {
      ...
      this.setState({
        cursorStyle:
          this.state.isScaleSetInProg && this.state.scalePts.length < 2
            ? 'crosshair'
            : 'move'
      });
    }
    ...
    if (this.state.isScaleSetInProg) {
      this.setState(
        prevState => ({
          scalePts: [
            ...prevState.scalePts,
            this.convertToImgPos(this.lastMouseUpPos)
          ]
        }),
        () => {
          this.setState({
  ...
  onClickDrawLine = () => {
    this.setState({ isDrawLineinProg: true, isScaleSetInProg: false }, () => {
